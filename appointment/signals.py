...
def notify_managers_appointment(sender, instance, created, **kwargs):
    ...
    send_mail(
        subject=subject,
        message=instance.message,
        from_email='dmitrushatest@yandex.ru',  # здесь указываете почту, с которой будете отправлять (об этом попозже)
        recipient_list=['dimas00095@yandex.ru']  # здесь список получателей. Например, секретарь, сам врач и т. д.
    )